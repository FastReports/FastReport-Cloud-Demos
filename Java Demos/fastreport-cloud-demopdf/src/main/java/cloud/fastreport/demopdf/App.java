/*
 * This Java source file was generated by the Gradle 'init' task.
 */
package cloud.fastreport.demopdf;

import cloud.fastreport.ApiClient;
import cloud.fastreport.ApiException;
import cloud.fastreport.Configuration;
import cloud.fastreport.client.DownloadApi;
import cloud.fastreport.client.ExportsApi;
import cloud.fastreport.client.TemplatesApi;
import cloud.fastreport.model.*;
import com.sun.net.httpserver.BasicAuthenticator;


import java.io.*;
import java.net.Authenticator;
import java.net.PasswordAuthentication;
import java.net.http.HttpClient;
import java.util.ArrayList;
import java.util.Base64;


public class App {
    public final static String report = "";

    public static void main(String[] args) {
        String apiKeyRaw = "apikey:***PUT YOUR API KEY HERE***";
        String apiKeyEncoded = Base64.getEncoder().encodeToString(apiKeyRaw.getBytes());

        ApiClient defaultClient = new ApiClient()
                .setBasePath("https://fastreport.cloud")
                .addDefaultHeader("Authorization", "Basic " + apiKeyEncoded);

        TemplatesApi templatesApi = new TemplatesApi(defaultClient);
        ExportsApi exportsApi = new ExportsApi(defaultClient);
        DownloadApi downloadApi = new DownloadApi(defaultClient);

        try {
            byte[] byteReport = Base64.getDecoder().decode(report);
            FileVM root = templatesApi.templateFoldersGetRootFolder(null);
            TemplateVM template = templatesApi.templatesUploadFileV2(root.getId(),
                    new File("Box.frx"), null, null);

            FileVM exportRoot = exportsApi.exportFoldersGetRootFolder(null);

            ExportVM export = templatesApi.templatesExport(template.getId(),
                    new ExportTemplateVM()
                            .fileName("java.pdf")
                            .format(ExportFormat.PDF)
                            .folderId(exportRoot.getId())
            );

            // wait until status ready

            while (export.getStatus() != FileStatus.SUCCESS) {
                export = exportsApi.exportsGetFile(export.getId());
                try {
                    Thread.sleep(100);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }

            File tempFile = downloadApi.downloadGetExport(export.getId(), false);
            System.out.println("filename = " + tempFile.getName());

            File file1 = new File("box.pdf");
            copyFileUsingStream(tempFile, file1);
        } catch (ApiException e) {
            e.printStackTrace();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    private static void copyFileUsingStream(File source, File dest) throws IOException {
        InputStream is = null;
        OutputStream os = null;
        try {
            is = new FileInputStream(source);
            os = new FileOutputStream(dest);
            byte[] buffer = new byte[1024];
            int length;
            while ((length = is.read(buffer)) > 0) {
                os.write(buffer, 0, length);
            }
        } finally {
            is.close();
            os.close();
        }
    }
}
